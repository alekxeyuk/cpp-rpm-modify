...
static size_t alignTo8Bytes(size_t offset) { return alignTo(offset, 8); }
static size_t alignTo4Bytes(size_t offset) { return alignTo(offset, 4); }
static size_t alignTo2Bytes(size_t offset) { return alignTo(offset, 2); }

static uint32_t add_signed_offset(uint32_t offset, ptrdiff_t delta) {
...
int main(int argc, char* argv[]) {
  ...
  for (auto& tag : head.index_entries) {
    if (tag.tag == 1011) {
      ...
      printf("[Old Vendor] [%u at %u] = [%s] [%zu]\n", tag.tag, tag.offset, old_vendor.c_str(), old_vendor.size());
    }
    if (tag.tag == 1015) {
      old_packager = mem_arr + tag.offset;
      printf("[Old Packager] [%u at %u] = [%s] [%zu]\n", tag.tag, tag.offset, old_packager.c_str(), old_packager.size());
    }
  }

  ptrdiff_t off_delta_vendor = static_cast<ptrdiff_t>(new_vendor_size) - static_cast<ptrdiff_t>(old_vendor.size());
  ptrdiff_t off_delta_packager = static_cast<ptrdiff_t>(new_packager_size) - static_cast<ptrdiff_t>(old_packager.size());
  printf("off_delta_vendor[%td] off_delta_packager[%td]\n", off_delta_vendor, off_delta_packager);

  file.seekp(header_index_off, ios::beg);

  size_t new_data_size = static_cast<size_t>(static_cast<ptrdiff_t>(head.data_size) + off_delta_vendor + off_delta_packager);
  ...
  ptrdiff_t align_offset = 0;
  for (auto& entry : head.index_entries) {
    size_t old_offset = entry.offset;

    printf("Entry [%d]\n", counter);
    printf("| Tag: %u | ", entry.tag);
    printf("Type: %u | ", entry.type);
    printf("Old Offset: %zu | ", old_offset);
    printf("New Offset: %td | ", static_cast<ptrdiff_t>(old_offset) + total_offset);
    printf("Count: %u |\n", entry.count);
    if (entry.tag == 63) {
      entry.offset = add_signed_offset(entry.offset, off_delta_vendor + off_delta_packager);
      memcpy(temp_im_63, mem_arr + old_offset, entry.count);
      temp_im_63_entry = entry;
    }


    entry.offset = add_signed_offset(entry.offset, total_offset + align_offset);
    if (entry.tag == 1011) {
      ...
      if (entry.type == 7) { // BIN
        ...
        size_t aligned = alignTo4Bytes(header_data_off + entry.offset) - header_data_off;
        if (aligned > entry.offset) {
          printf("[LAST OFFSET] = [%u] [ALLIGNED OFFSET] = [%zu]\n", entry.offset, aligned);
          align_offset += static_cast<ptrdiff_t>(aligned - entry.offset);
          entry.offset = add_signed_offset(entry.offset, align_offset);
        }
        ...
        size_t aligned = alignTo2Bytes(header_data_off + entry.offset) - header_data_off;
        if (aligned > entry.offset) {
          printf("[LAST OFFSET] = [%u] [ALLIGNED OFFSET] = [%zu]\n", entry.offset, aligned);
          align_offset += static_cast<ptrdiff_t>(aligned - entry.offset);
          entry.offset = add_signed_offset(entry.offset, align_offset);
  ...
  file.write(new_mem_arr, static_cast<streamsize>(new_data_size + align_offset));
  file.write(data_mem_arr, rpm_data_size);


  file.seekp(header_index_off, ios::beg);
  head.data_size = static_cast<uint32_t>(new_data_size + align_offset);
  head.byte_swap();
  file.write(reinterpret_cast<char*>(&head), 16);


  temp_im_63_entry.offset = add_signed_offset(temp_im_63_entry.offset, align_offset);
